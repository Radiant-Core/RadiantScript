...
export default class GenerateTargetTraversal extends AstTraversal {
  ...
  private withNode<T extends Node>(node: T, fn: () => void): T {
  ...
  private emit(op: OpOrData | OpOrData[]): void {
    // Record source location before emitting if in debug mode
    if (this.debugMode && this.currentNode?.location) {
      const bytecodeOffset = this.output.length;
      const loc = this.currentNode.location;
      const entry: SourceMapEntry = {
        line: loc.start.line,
        column: loc.start.column,
        functionName: this.currentFunction?.name,
      };
      this.sourceMap[bytecodeOffset] = entry;
    }
