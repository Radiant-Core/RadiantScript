...
#### Example
```ts
.to('radiant:qrhea03074073ff3zv9whh0nggxc7k03ssh8jv9mkx', 500000)
```
...
The `from()` function allows you to provide a hardcoded list of contract UTXOs to be used in the transaction. This overrides the regular UTXO selection performed by the RadiantScript SDK, so **no further selection will be performed** on the provided UTXOs. This function can be called any number of times, and the provided UTXOs will be added to the list of earlier added UTXOs.
...
The `withHardcodedFee()` function allows you to specify a hardcoded fee to the transaction. By default the transaction fee is automatically calculated by the RadiantScript SDK, but there are certain use cases where the smart contract relies on a hardcoded fee.
...
After completing a transaction, the `send()` function can be used to send the transaction to the RXD network. An incomplete transaction cannot be sent.
...
const txDetails = await instance.functions
  ...
  .withOpReturn(['0x6d02', 'Hello World!'])
  .to('radiant:qrhea03074073ff3zv9whh0nggxc7k03ssh8jv9mkx', 200000)
  .to('radiant:qqeht8vnwag20yv8dvtcrd4ujx09fwxwsqqqw93w88', 100000)
  .withHardcodedFee(1000)
  .send()
```
