...
RadiantScript is a statically typed language, which means that the type of each variable needs to be specified. Types can also be implicitly or explicitly cast to other types. For a quick reference of the various casting possibilities, see [Type Casting](#type-casting).
...
Some byte sequences hold specific meanings inside Radiant contracts. These have been granted their own types separate from the regular `bytes` type.
...
```solidity
string question = "What is Radiant?";
