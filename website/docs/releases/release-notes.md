...
* :bug: Fix bug where certain covenants could become unspendable due to incorrect bytesize calculation
  * :boom: **BREAKING**: Covenants using `tx.bytecode` now include a placeholder `OP_NOP` that gets replaced when constructor arguments are provided in the RadiantScript SDK. If you're not using the RadiantScript SDK, refer to the [`replaceBytecodeNop()` function](https://github.com/Radiant-Core/RadiantScript/blob/master/packages/utils/src/script.ts#L130) to see the steps required to do so manually.
...
#### RadiantScript SDK
* :sparkles: Add BitcoinRpcNetworkProvider that connects to a RXD node RPC
* :boom: **BREAKING**: Remove dependency on `rxdc` and remove `CashCompiler` export
...
## v0.5.0
#### RadiantScript SDK
RadiantScript used to be very tightly coupled with BITBOX. This proved to be problematic after maintenance for BITBOX was stopped. The main objective of this update is to allow RadiantScript to be used with many different RXD libraries.
...
- :boom: **BREAKING**: Refactor contract instantiation flow
  ...
  - Anyone can implement the NetworkProvider interface to create a custom provider. The RadiantScript SDK offers three providers out of the box: one based on electrum-cash (default), one based on FullStack.rxd' infrastructure, and one based on BITBOX. See the [NetworkProvider docs](/docs/sdk/instantiation#networkprovider) for details.
...
- :boom: **BREAKING**: Remove the artifacts `'networks'` field and `.deployed()` functionality, This proved to be confusing and is better suited to be handled outside of the RadiantScript SDK.
...
#### RadiantScript SDK
- :sparkles: Add ability to provide hardcoded inputs to the transaction rather than use RadiantScript's coin selection.
...
- :shell: CLI improvements
  ...
  - Add `--args|-a` flag that allows you to specify constructor arguments that are added to the generated bytecode.
    - :warning: The CLI **does not** perform type checking on these arguments, so it is recommended to use the RadiantScript SDK for type safety.
...
#### RadiantScript SDK
- :globe_with_meridians: Browser support! You can now use RadiantScript inside web projects. Filesystem-based functionality such as compilation from file are not supported due to the nature of web, so RadiantScript files have to be read in a different way (e.g. Fetch API) and then passed into the RadiantScript SDK.
...
* :sparkles: Covenants abstraction! All individual preimage fields can be accessed without manual decoding, passing, and verification.
  ...
  * When any of these fields is used inside a function, this function is marked `covenant: true`, and requires a preimage as parameter (automatically passed by RadiantScript SDK).
...
* :name_badge: Renamed `Contract.fromCashFile` to `Contract.compile`.
  * The new function allows to pass in a path to a `.rxd` file, or a string of the contract source code.
...
* :sparkles: Add `pragma` keyword to specify intended compiler version.
  * Example: `pragma radiantscript ^0.2.0;`
...
#### RadiantScript SDK
* :arrow_up: Update `rxdc` and other dependencies.
